    ...
    for class_combination in classCombinations:
        ...
        for class_number in class_combination:
            ...
            for filename in glob.glob(
                ...
                #texture
                texture_features=np.genfromtxt(base_texture_training + str(class_number) + '/'+name)
                ...
                texture_model.all_features = np.append(texture_model.all_features,
                                                      ...
                                                      1, texture_model.num_blocks_per_class * texture_model.num_features)))
                texture_model.num_training_examples += texture_model.num_blocks_per_class

                #sift
                name= name.replace(".csv",".jpg")
                SDS_class = np.genfromtxt(base_SDS_training+name, delimiter=",")
                SDS_class = SDS_class.reshape((1, SDS_class.shape[0]))
                SOH_class = np.genfromtxt(base_SOH_training+name, delimiter=",")
        ...
        for class_number in class_combination:
            for filename in glob.glob(
                ...
                prediction_horest = np.average(horest_model.classifier.predict_proba(horest_features_test), axis=0).reshape(1, -1)
                classes_horest = horest_model.get_classifier_classes()
                horest_indecies_sorted = np.argsort(classes_horest, axis=0)
                sorted_horest_classes = classes_horest[horest_indecies_sorted[::-1]]
                sorted_horest_predictions = prediction_horest[horest_indecies_sorted[::-1]]
                ...
                prediction_texture = np.average(texture_model.classifier.predict_proba(texture_features_test), axis=0).reshape(1, -1)
                ...
                score[np.argwhere(sorted_horest_classes == prediction_sift)] += (1 / 2)
        ...
        if testcases > 5000:
            ...
            np.savetxt('ar_sift_validation.csv', accuracy, delimiter=',')
            break
