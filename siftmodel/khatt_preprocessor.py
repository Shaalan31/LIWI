...
base = 'C:/Users/omars/Documents/Github/LIWI/'
#KHATT preprocessor
def data_preprocessor():
    sift = Sift()
    with open("ArabicOutput.txt", 'w') as out:
        out.write("")
    batch_num = 0
    SDpoints = np.zeros((1,128))
    for filename in glob.glob(base+'Omar/KHATT/*/*/*.tif'):
        ...
        if SDpoints.shape[0] > 40000:
            ...
            with h5py.File(base+'Datasets/KHATT/SDpoints'+str(batch_num) +'.h5', 'w') as hf:
...
def normalize():
    ...
    for x in range(0,40):
        print(x)
        with h5py.File(base+'Datasets/KHATT/SDpoints'+str(x) +'.h5', 'r') as hf:
    ...
    np.savetxt(base+'Datasets/KHATT/mean.csv', mean, delimiter=",")
    np.savetxt(base+'Datasets/KHATT/deviation.csv', deviation, delimiter=",")


    with h5py.File(base+'Datasets/KHATT/AData.h5', 'w') as hf:
